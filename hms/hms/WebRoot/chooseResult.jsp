...
<div class="container hms_container">
	<div class="hms_title">
		<img alt="black_block" src="./image/black_block.png"
			class="black_block" /> <span class="board_block">${hms_title}（${person.p0_name }）</span>
	</div>
	<div class="hms_main">
		<div class="choose_result">
			<c:if test="${person.choose_state == 1 }">
				<span class="choose_result_lable" style="margin-left: 0;">您抽中的号码是</span>
				<span class="label label-success">${person.choose1_result }</span>
			</c:if>
			<c:if test="${person.choose_state == 2 }">
				<span class="choose_result_lable" style="margin-left: 0;">您抽中的号码是</span>
				<span class="label label-success">${person.choose2_result }</span>
				<span class="choose_result_lable">您抽中的房号是</span>
				<span class="label label-success">${person.newHouse.house_no
					}</span>
			</c:if>
		</div>
		<hr />
		...
		<h3 class="search_detail_title">
			抽签信息<span class="label ${choose_state_class }">${choose_state
				}</span>
		</h3>
		...
		<hr />
		<h3 class="search_detail_title">
			人员信息<span class="label ${sign_state_class }">${sign_state }</span>
		</h3>
